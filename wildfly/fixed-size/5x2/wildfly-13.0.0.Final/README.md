# WildFly 13 Domain (5x2)

Contains a Dockerfile to start 
 
- a stock WildFly 13 domain controller with five server groups `server-group-0` .. `server-group-4` using profile `default` and no servers attached to it.
...
The image is not published to the public docker registry. To build the image use
    ...
    docker build -t wildfly-13-5x2 .

## Run

To start the domain controller use 

    docker run --rm -it -p 9990:9990 --name=dc wildfly-13-5x2 --host-config host-master.xml -b 0.0.0.0 -bmanagement 0.0.0.0
    
Use `admin`:`admin` to connect to the management console on the domain controller. 
    
To start a host controller use

    docker run --rm -it --link dc:domain-controller wildfly-13-5x2 --host-config host-slave.xml
